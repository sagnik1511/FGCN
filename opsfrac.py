...
def CONV2D(x,name,shape,stride=[1,1,1,1],padding="SAME"):
    c_filter = tf.compat.v1.get_variable(name,shape,"float32",tf.compat.v1.truncated_normal_initializer())
    return tf.nn.conv2d(input=x,filters=c_filter,strides=stride,padding=padding)
...
def filter_variable(name,shape,trainable=True):
    return tf.compat.v1.get_variable(name,shape,"float32",trainable=trainable,initializer=tf.compat.v1.truncated_normal_initializer())